...
private:
	...
	[[nodiscard]] static AstSemanticValue BuildAddressOfEmptyStructLiteral(const std::vector<AstSemanticValue>& values);
	...
	[[nodiscard]] static AstSemanticValue PassToken(const std::vector<AstSemanticValue>& values);
	...
	[[nodiscard]] static std::vector<std::string> TakeIdentifierNamesFromTargets(std::vector<AstSemanticValue>& values);
